		var schedules = [
			{week: 1, ename: '해안가감찰', epic: 0, echar: [24], eweapon: [65], cat: 'event', guide: 'scout-coast', etc: ''},
			{week: 1, ename: '전술장비 작동', epic: 1, echar: [0, 2, 9], eweapon: [], cat: 'gacha', guide: '', etc: ''},
		];

		add_schedule(3, '급습 앨리스', 2, [78], [67], 'event', 'raid-alice','');
		add_schedule(3, '해변의 로맨스', 3, [64, 71, 88], [], 'gacha', '', '');
		add_schedule(1, '기적의 서약 업데이트', 4, [1, 8], [], 'pledge', 'oath-of-beauty', '');
		add_schedule(6, '봉인작전', 5, [89], [32], 'event', 'sealing-operation', '');
		add_schedule(6, '전술장비 업그레이드', 6, [45, 51, 79], [], 'gacha', '', '');
		add_schedule(6, '유쾌한 할로윈 주간', 7, [], [68], 'event', '', '', '');
		add_schedule(9, '파자마 파티', 8, [62, 63, 84], [], 'gacha', '', '');
		add_schedule(9, '소녀들의 담력 테스트', 9, [92], [69], 'event', 'testofcourage', '');
		add_schedule(11, '역전 체크메이트', 10, [65, 39, 76], [], 'gacha', '', '');
		add_schedule(11, '결선 바디체스', 11, [80], [70], 'event', 'bodychess', '');
		add_schedule(14, '겨울파티 무기뽑기', 12, [], [71, 72], 'weapongacha', '', '');
		add_schedule(14, '겨울 홀리데이', 13, [40, 41, 85], [], 'gacha', '', '');
		add_schedule(14, '화이트 크리스마스', 14, [90], [], 'event', 'whitechristmas', '', '');
		add_schedule(14, '기적의 서약 업데이트', 15, [44, 37], [], 'pledge', 'oath-of-beauty', '');
		add_schedule(15, '물가의 바캉스', 16, [52, 66, 77], [], 'gacha', '', '');
		add_schedule(15, '물가의 스펙트럼', 17, [91], [73], 'event', 'event-spectrum', '');
		add_schedule(18, '춘절 문 업데이트', 18, [46], [], 'update', '', '');
		add_schedule(18, '춘절2020', 19, [42, 50], [], 'gacha', '', '');
		add_schedule(18, '춘절 무기뽑기', 20, [], [74, 75], 'weapongacha', '', '');
		add_schedule(18, '황야여단', 21, [], [], 'event', 'desertbrigade', '', '');
		add_schedule(21, '소악마 데이트', 22, [53], [], 'gacha', '', '');
		add_schedule(22, '전술장비 포워드', 23, [59, 72, 97], [], 'gacha', '', '');
		add_schedule(22, '베이 지역 괴멸', 24, [86], [76], 'event', 'bayArea_destruction', '');
		add_schedule(24, '사무관의 일', 25, [67, 47, 98], [], 'gacha', '', '');
		add_schedule(24, 'Prove it, Please', 26, [81], [77], 'event', 'proveitplease', '');
		add_schedule(26, '기략종횡무기', 27, [], [78, 79], 'weapongacha', '', '');
		add_schedule(26, '기갑공격 뽑기', 28, [48, 55], [], 'gacha', '', '');
		add_schedule(26, '새소녀 퀘스트', 29, [99], [], 'event', '', '');
		add_schedule(28, '해사장 비너스', 30, [56, 49, 82], [], 'gacha', '', '');
		add_schedule(28, '해치의 철인', 31, [83], [80], 'event', 'beachIronman', '');
		add_schedule(30, '신록의 나뭇잎 새는 날', 32, [51, 0, 9, 76], [], 'gacha', '', '');
		add_schedule(32, '픽업무기뽑기', 33, [], [], 'weapongacha', '', ''); //6, 5, 2, 
		add_schedule(32, '왕자님 따위 필요 없어', 34, [74, 73, 93], [], 'gacha', '', '');
		add_schedule(32, 'RoSE\'s Fairy Tale', 35, [94], [81], 'event', 'rose-sfairytale', '');
		add_schedule(32, '기적의 서약 업데이트', 36, [57, 61], [], 'pledge', 'oath-of-beauty', '');
		add_schedule(35, '소녀의 택틱스 mission.1', 37, [], [], 'event', 'girltactics', '');
		add_schedule(35, '봄바람 심포니', 38, [38, 43], [], 'gacha', '', '');
		add_schedule(38, '복각: 해안가감찰', 39, [24], [], 'event', 'scout-coast', '');
		add_schedule(38, '복각: 전술장비 작동', 41, [0, 2, 9], [], 'gacha', '', '');
		add_schedule(38, '중력 커스터마이즈 무기뽑기', 40, [], [82, 83], 'weapongacha', '', '');
		add_schedule(41, '소녀의 택틱스 mission.2', 42, [], [], 'event', 'girltactics-2', '');
		add_schedule(41, '한여름 에튀드 특별 뽑기', 43, [68, 75], [], 'gacha', '', '');
		add_schedule(43, '운명의 만남', 44, [], [], 'gacha', '', '');
		add_schedule(45, '울트라 마린의 여름 하늘 뽑기', 46, [104, 105, 103], [], 'gacha', '', '');
		add_schedule(45, '군청의 날개', 47, [102], [], 'event', 'theultramarinewings', '');
		add_schedule(45, '마음을 연주하는 손가락 끝', 48, [], [84, 85], 'weapongacha', '', '');



		add_schedule(46, '아이린 생일 빙고 1/18', '0118', [58], [], 'event', '', '');
		add_schedule(46, '소쇼우신 생일 빙고 2/12', '0212', [54], [], 'event', '', '');
		add_schedule(46, '주노 생일 빙고 4/1', '0401', [60], [], 'event', '', '');
		add_schedule(46, '지무카 생일 빙고 5/14', '0514', [69], [], 'event', '', '');
		add_schedule(46, '로코코 생일 빙고 6/14', '0614', [70], [], 'event', '', '');
		add_schedule(46, '그루니에 생일 빙고 7/6', '0706', [101], [], 'event', '', '');
		add_schedule(46, '리타 생일 빙고 7/30', '0730', [106], [], 'event', '', '');



		function add_schedule(week, ename, epic, echar, eweapon, cat, guide, etc){
			schedules.push({week, ename, epic, echar, eweapon, cat, guide, etc});
		}

		function get_guide_addr(i){
			if(schedules[i].cat == 'event' || schedules[i].cat == 'pledge')
				var temp = 'https://papago.naver.net/website?locale=ko&source=ja&target=ko&url=https%3A%2F%2Fgirlcafegun.boom-app.wiki%2Fentry%2F' + schedules[i].guide;
			return temp;
		}


		for(let i in schedules){
			if(schedules[i].ename == '춘절 문 업데이트' || schedules[i].ename == '춘절2020' || schedules[i].ename == '춘절 무기뽑기' || schedules[i].ename == '황야여단'){
				schedules[i].week = 6;
			}
			else if(schedules[i].week >= 6){
				schedules[i].week += 3;
			}
		}